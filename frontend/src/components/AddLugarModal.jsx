...
const inputCls =
  "w-full border-2 border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder:text-slate-300 outline-none focus:border-[#1c16cd] transition-colors";

const AddLugarModal = ({ onClose }) => {
  const [enviado, setEnviado] = useState(false);
  const [categorias, setCategorias] = useState([]); // ← agrega
  const [loading, setLoading] = useState(false); // ← agrega
  const [error, setError] = useState(null); // ← agrega
  ...
  const puedeEnviar =
    form.nombre.trim() && form.direccion.trim() && form.descripcion.trim();
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await lugaresService.proponer({
        nombre: form.nombre,
        categoria: categoriaObj?._id,
        direccion: form.direccion,
        descripcion: form.descripcion,
        horario: form.horarioResumido,
        url_img: form.imagen,
        url_sitioweb: form.sitioWeb,
        caracteristicas_accesibilidad: form.accesibilidad.map((t) =>
          t.toLowerCase(),
        ),
      });
      setEnviado(true);
        ...
        {enviado ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center gap-4">
            <div className="w-20 h-20 bg-[#13da28]/10 rounded-full flex items-center justify-center">
              <span
                className="material-symbols-rounded text-[#13da28]"
                style={{ fontSize: "44px" }}
              >
                check_circle
              </span>
            ...
            <p className="text-slate-500 text-sm max-w-sm">
              Tu lugar fue enviado para revisión. Nuestro equipo lo revisará y
              lo publicará pronto.
            </p>
            <button
              onClick={onClose}
          ...
          <>
            ...
            <div className="px-6 pt-6 pb-4 border-b border-slate-100 pr-16">
              <h2 className="font-extrabold text-xl text-[#171717]">
                Agregar lugar
              </h2>
              <p className="text-slate-500 text-sm mt-0.5">
                Comparte un lugar accesible con la comunidad
              </p>
            </div>
                      ...
                      <button
                        ...
                        className={`text-xs font-semibold px-3 py-1.5 rounded-full transition-all ${
                          form.categoria === cat.nombre
                            ? `${CATEGORIA_COLORS[cat.nombre]} text-white shadow-sm`
